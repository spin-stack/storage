...
func TestASecondPublisherIsRefusedRatherThanOverwriting(t *testing.T) {
	...
	etag, err := image.Publish(ctx, store, vol, first, 1, "")
	if err != nil {
	...
	if _, err := image.Publish(ctx, store, vol, second, 1, ""); !errors.Is(err, image.ErrSuperseded) {
	...
	loaded, _, _, err := image.Load(ctx, store, vol)
	if err != nil {
		t.Fatal(err)
	}
	got := make([]byte, 512)
	...
	if _, err := image.Publish(ctx, store, vol, third, 2, etag); err != nil {
...
func TestAStaleETagIsRefused(t *testing.T) {
	...
	stale, err := image.Publish(ctx, store, vol, v, 1, "")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := image.Publish(ctx, store, vol, v, 2, stale); err != nil {
		t.Fatal(err)
	}
	if _, err := image.Publish(ctx, store, vol, v, 3, stale); !errors.Is(err, image.ErrSuperseded) {
...
func TestAVolumeWithNoImageIsNotAFailure(t *testing.T) {
	if _, _, _, err := image.Load(t.Context(), sim.NewObjectStore(), vol7()); !errors.Is(err, image.ErrNotPublished) {
