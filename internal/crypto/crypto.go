...
//
// A layer's frame nonces are derived deterministically from the layer's own identity and
// the frame index (§10, layerNonce) and never stored — a layer id is minted once, so no
// nonce is reused. Only DEK generation and DEK wrapping consume randomness, and both take
// an injected io.Reader.
package crypto
