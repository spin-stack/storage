...
| DEV-0024 | Is the 64 KiB CoW segment granularity V2, or dead? It exists nowhere: `cow.IntervalMap` has no grid, and what leaves the host is a content-addressed chunk of up to `image.MaxChunkBytes`. The schema comment that invented it is corrected; `block_size` is the guest's logical block size, and reaches the guest through `vhost.Config.BlockSize` — not through `blockdev`, which has no block size at all. | **open** → STATUS.md, narrowed 2026-08-07. Downstream of `CHUNK-ADDRESSING-SPEC.md`, whose question is now answered *structure* — which makes the addressed unit part of the decision |
...
| DEV-0023 | The design document promised resize in the present tense after `metadata.Store.ResizeVolume` was deleted — §3's objective 14, §4's durable-volume row, §6's client column, §7 twice, §8's `size_bytes` comment and the `operations` kind list, §9's `resize2fs` sentence, §18, §30's item 9, §31's criterion 16 and §32's diagram. | resolved `d315840` — **resize is V2.** §3's objective 14 carries the reasoning; every other mention is struck through and points there. The half that could not be built is the one to remember: `vhost.ProtocolFeatures` does not advertise the backend request channel a `VHOST_USER_BACKEND_CONFIG_CHANGE_MSG` travels on |
