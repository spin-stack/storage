...
| DEV-0024 | `internal/schema/schema.sql` — the declared source of truth — comments `block_size` as the 64 KiB CoW segment granularity. It is the guest's logical block size (`VolumeSpec.BlockSize`, refused unless a multiple of the 512-byte sector, defaulted to 4096 by `-seed-block-size`), and the 64 KiB CoW segment does not exist anywhere: what leaves the host is a content-addressed chunk of up to `image.MaxChunkBytes`. | **open** → STATUS.md. The comment is track D's file, and whether the 64 KiB granularity is V2 or dead is downstream of `CHUNK-ADDRESSING-SPEC.md` and DEV-0020 |
