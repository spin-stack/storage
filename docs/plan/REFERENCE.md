...
| ADR-0026 | **Proposed, not accepted.** V1 accepts an RPO of one session: §14.8 local becomes the only ACK contract and a volume is uploaded once, at stop. Withdraws the remote durability chain. Blocked on one product question — has anyone asked for a VM to survive host loss mid-session? | Proposed |
