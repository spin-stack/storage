...
| DEV-0018 | Three documents claimed a Linux-guest lane that no test performed — every `integration/vhost` test boots a 512-byte SeaBIOS boot sector, and INT 13h has no flush verb. The lane now exists and is skipped: a real kernel registers the device, reports the right capacity, and then hangs on its first block request. | **open** → STATUS.md |
