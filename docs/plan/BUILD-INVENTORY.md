...
| `Fenced()` → tear the runtime down | `loop.go` + `volume.go` | `report()` computes the fenced list and its own comment says the SELF_FENCED transition "belongs to the data path". Resolves DEV-0012's Agent half — the trigger that stops a guest's I/O. The log's own self-fencing stops only the durable path, which §12.2 permits. | M | **yes (fencing)** |
